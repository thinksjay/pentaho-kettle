...
public class TextFileInputReader implements IBaseFileInputReader {
    ...
    protected void readInitial() throws Exception {
        ...
        //如果没有添加文件数据输出字段，则输出全部
        if (meta.getAutoGenerationFileField() && meta.content.header) {
            TextFileLine headerLine = data.lineBuffer.get(meta.content.nrHeaderLines - 1);
            String[] headerFields = headerLine.getLine().split(meta.content.separator);

            //清除之前自动生成的字段
            if(meta.inputFields!=null && meta.inputFields.length>0)
            {
                int fieldIndex=-1;
                for(BaseFileField fileField : meta.inputFields)
                {
                    fieldIndex = data.outputRowMeta.indexOfValue(fileField.getName());
                    if(fieldIndex>-1)
                    {
                        data.outputRowMeta.removeValueMeta(fieldIndex);
                    }
                }
            }

            //自动生成字段
            meta.inputFields=new BaseFileField[headerFields.length];

            for (int i=0;i< headerFields.length;i++) {
                data.outputRowMeta.addValueMeta(new ValueMetaString(headerFields[i]));
                meta.inputFields[i]=new BaseFileField(headerFields[i],i,0);
            }
        ...
        if(meta.inputFields!=null && meta.inputFields.length>0 )
        {
            data.fildFields=String.join("," ,Arrays.stream(meta.inputFields).map(t->t.getName()).collect(Collectors.toList()));
        }
                ...
                if (data.pageLinesRead < meta.content.nrLinesPerPage) {
                    ...
                    r =
                            TextFileInputUtils.convertLineToRow(log, textLine, meta, data.currentPassThruFieldsRow,
                                    ...
                                    data.lastModificationDateTime, data.uriName, data.rootUriName,data.fildFields, data.extension, data.size);
                    if (r != null) {
                        putrow = true;
                    }
